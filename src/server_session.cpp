...
void server_session_t::in_recv_at_handshake(const std::string& buf)
{
    anole::request_t req;
    ...
    sess_.resolver.async_resolve(query_addr, query_port, [this, self, query_addr, query_port](const boost::system::error_code err, boost::asio::ip::tcp::resolver::results_type rc){
        post_resolve_remote(query_addr, query_port, err, rc);
    });
}

void server_session_t::post_resolve_remote(const std::string& query_addr, const std::string& query_port, const boost::system::error_code err, boost::asio::ip::tcp::resolver::results_type rc)
{
    if (err || rc.size() < 1)
    {
